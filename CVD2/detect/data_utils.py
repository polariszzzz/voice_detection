...
import numpy as np
from joblib import Parallel, delayed #导出模型
import h5py
...
ASVFile = collections.namedtuple('ASVFile',
    ['speaker_id', 'file_name', 'path', 'sys_id', 'key']) #定义了ASVFile的元组类型

class ASVDataset(Dataset): #必须要重载__len__以及__getitem__
    """ Utility class to load  train/dev datatsets """
    def __init__(self, transform=None, 
        is_train=True, sample_size=None, 
        is_logical=True, feature_name=None, is_eval=False,file='121.flac',
        eval_part=0):
        if is_logical:
        ...
        if is_eval:
            '''self.files_meta = [ASVFile(speaker_id='',
            file_name='',
            path='evaluate/female1.flac',
            sys_id='',
            key=1),ASVFile(speaker_id='',
            file_name='',
            path='evaluate/synthesized_voice.wav',
            sys_id='',
            key=0),ASVFile(speaker_id='',
            file_name='',
            path='evaluate/synthesized_voice2.wav',
            sys_id='',
            key=0)]  # 返回自定义类型ASVFile的列表'''
            self.files_meta = []
            path=('/backup1/lx/CVD/uploads/'+file)
            a=[ASVFile(speaker_id='', file_name='',
                                               path=path,
                                               sys_id='',
                                               key=1),]
            for i in a:
                self.files_meta.append(i)


            data = list(map(self.read_file, self.files_meta))
            print(self.files_meta[0].path)
        ...
        elif os.path.exists(self.cache_fname):
            ...
            self.files_meta = self.parse_protocols_file(self.protocols_fname) #返回自定义类型ASVFile的列表
            data = list(map(self.read_file, self.files_meta))
            self.data_x, self.data_y, self.data_sysid = map(list, zip(*data))
            if self.transform:
                # self.data_x = list(map(self.transform, self.data_x)) 
    ...
    def read_file(self, meta):
        data_x, sample_rate = sf.read(meta.path) #soundfile为读取音频文件的库
        data_y = meta.key
        return data_x, float(data_y), meta.sys_id

    def _parse_line(self, line): #分析行
        tokens = line.strip().split(' ')  #每行删除空白符，用‘ ’进行分割
        '''if self.is_eval:
        ...
        return ASVFile(speaker_id=tokens[0],
            ...
            key=int(tokens[4] == 'bonafide')) #key表示是否是真实的语音（训练时）

    def parse_protocols_file(self, protocols_fname): #解析文件
