...
def train_epoch(data_loader, model, lr, device): #训练一轮,edited data_loader
    running_loss = 0
    num_correct = 0.0
    num_total = 0.0
    ii = 0
    model.train() #训练模式
    optim = torch.optim.Adam(model.parameters(), lr=lr)  #（待优化的参数，学习率）
    weight = torch.FloatTensor([1.0, 9.0]).to(device)
    criterion = nn.NLLLoss(weight=weight)  #损失函数
    for batch_x, batch_y, batch_meta in train_loader:
        ...
        optim.zero_grad()  #梯度置零，loss关于weight的导数置零
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,
                        default='models.model_logical_spect_5_32_5e-05.epoch_4.pth', help='Model checkpoint')
    parser.add_argument('--eval_output', type=str, default='evaluate_result.result.txt',
                        help='Path to save the evaluation result')
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--num_epochs', type=int, default=5) #轮数
    ...
    train_set = data_utils.ASVDataset(is_train=True, is_logical=is_logical, transform=transforms,
                                      feature_name=args.features) #从data_utils.py(文件)获得ASVDataset

    train_loader = DataLoader(
        train_set, batch_size=args.batch_size, shuffle=True) #从torch.utils.data(Pytorch的库)运用DataLoader加载数据
    num_epochs = args.num_epochs #数据集轮的次数
    writer = SummaryWriter('logs/{}'.format(model_tag))
    for epoch in range(num_epochs):
        running_loss, train_accuracy = train_epoch(
            train_loader, model, args.lr, device) #学习一轮
