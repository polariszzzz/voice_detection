...
def evaluate_accuracy(data_loader, model, device):
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        #print('prediction matrix is',batch_pred)
        #print('real matrix is',batch_y)
...
def channel(filename):
    ...
    dev_set = data_utils.ASVDataset(is_train=False, is_logical=is_logical,
                                    ...
                                    feature_name=args.features, is_eval=True, eval_part=args.eval_part,file=filename)
    dev_loader = DataLoader(dev_set, batch_size=args.batch_size, shuffle=True)
    model = model_cls().to(device)

    model.load_state_dict(torch.load('/backup1/datas/models/students/epoch_19.pth'))
